...
# Make directories
dir.create(sprintf("decoupler_workflow/prune_edges/%s", enrich_slot), recursive = TRUE, showWarnings = FALSE)
dir.create(sprintf("decoupler_workflow/prune_edges/%s/out_files", enrich_slot), recursive = TRUE, showWarnings = FALSE)
...
# Add noise column
joined_results$noise <- sprintf("prune_edges_frac%s_rep%s", noise_perc, n_rep)
auc_sum$noise <- sprintf("prune_edges_frac%s_rep%s", noise_perc, n_rep)
