...
n_rep = args[8]
enrich_slot = args[9]
netw_fname_tsv = args[10]

# Make directories
dir.create(sprintf("decoupler_workflow/shuffle_edges/%s", enrich_slot), recursive = TRUE, showWarnings = FALSE)
dir.create(sprintf("decoupler_workflow/shuffle_edges/%s/out_files", enrich_slot), recursive = TRUE, showWarnings = FALSE)
...
opts_list <- list(list(
  ...
  enricher = list(scaler_type="robust", minsize=5, enr_type=enrich_slot, regulon=netw_fname_tsv)
))
...
# Add noise column
joined_results$noise <- sprintf("shuffle_edges_frac%s_rep%s", noise_perc, n_rep)
auc_sum$noise <- sprintf("shuffle_edges_frac%s_rep%s", noise_perc, n_rep)
